...
export default function CharacterSettingsModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm animate-fade-in transition-all">
      ...
      <div className="relative w-full max-w-sm h-full bg-zinc-950 border-l border-white/10 p-6 flex flex-col shadow-2xl overflow-hidden animate-slide-left">
        <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-white flex items-center gap-2">⚙️ 도감 엔진 설정</h2>
            <p className="text-[11px] text-zinc-500 mt-1">캐릭터 랭크에 따라 블룸 페스 보너스가 갱신됩니다.</p>
          </div>
                ...
                {unit.members.map((char) => {
                  ...
                  return (
                    <div key={char} className="flex items-center justify-between bg-zinc-900/30 border border-white/5 rounded-xl p-2.5 hover:border-white/20 transition-colors">
                      <span className="text-[13px] font-semibold text-zinc-200">{char}</span>
                      
                      <div className="flex items-center gap-2.5">
                        <span className="text-[10px] font-mono text-pink-400 font-bold bg-pink-500/10 px-1.5 py-0.5 rounded shadow-sm">
                          +{bloomBonus}%
                        </span>
