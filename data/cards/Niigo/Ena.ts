import { defineCharacterCards } from "../template"; // 경로가 다르면 수정해 주세요!

// 🎨 25시, 나이트코드에서. (니고) 에나
// -> 얼굴 아이콘: Ena.png 자동 매핑
const EnaNiigo = defineCharacterCards("니고", "시노노메 에나", "Niigo", "Ena", {
  cards: [
        {
          info: {
            id: "ng_Ena_001",
            cardName: "[시노노메가의 연말]",
            attribute: "pure",
            gachaType: "통상",
            gachaPoolName: "세카이의 두근두근 새해 뽑기",
            eventName: "세카이의 해피 뉴 이어!",
            skillType: "퍼스업",
            releaseDate: "2022-07-24",
          },
          media: {
            gachaBannerPath: "/gachas/22y/banner_22_10.png",
            eventBannerPath: "/events/22y/banner_22_09.png",
          },
        },
        {
          info: {
            id: "ng_Ena_002",
            cardName: "[포기하고 싶지 않으니까]",
            attribute: "happy",
            gachaType: "통상",
            gachaPoolName: "빛이 내리쬐는 캔버스 뽑기",
            eventName: "채워지지 않는 페일 컬러",
            skillType: "스업",
            releaseDate: "2022-08-25",
          },
          media: {
            gachaBannerPath: "/gachas/22y/banner_22_14_n.png",
            eventBannerPath: "/events/22y/banner_22_13_n.png",
            songName: "끝없이 잿빛으로",
            songJacketPath: "/jacket/22y/Close_to_Gray.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "나르키소스 페인터 드레스",
          },
        },
        {
          info: {
            id: "ng_Ena_003",
            cardName: "[미스터리 투어의 세례?]",
            attribute: "cute",
            gachaType: "통상",
            gachaPoolName: "비밀의 드레스 룸 뽑기",
            eventName: "시크릿 디스턴스",
            skillType: "퍼스업",
            releaseDate: "2022-10-10",
          },
          media: {
            gachaBannerPath: "/gachas/22y/banner_22_20_n.png",
            eventBannerPath: "/events/22y/banner_22_19_n.png",
            songName: "아이디 스마일",
            songJacketPath: "/jacket/22y/IDSMILE.png",
          },
        },
        {
          info: {
            id: "ng_Ena_004",
            cardName: "[내밀어진 손]",
            attribute: "pure",
            gachaType: "통상",
            gachaPoolName: "포레스트 페어리 뽑기",
            eventName: "고민을 들려줘! 두근두근 피크닉",
            skillType: "힐",
            releaseDate: "2022-10-31",
          },
          media: {
            gachaBannerPath: "/gachas/22y/banner_22_23.png",
            eventBannerPath: "/events/22y/banner_22_22.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "페어리 테일",
          },
        },
        {
          info: {
            id: "ng_Ena_005",
            cardName: "[언젠가의 여름 축제]",
            attribute: "cool",
            gachaType: "통상",
            gachaPoolName: "여름밤에 떠오르는 지난날 뽑기",
            eventName: "여름 축제, 울려 퍼지는 소리는",
            skillType: "판강",
            releaseDate: "2022-12-19",
          },
          media: {
            gachaBannerPath: "/gachas/22y/banner_22_30.png",
            eventBannerPath: "/events/22y/banner_22_29.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "서늘한 황혼의 옷",
          },
        },
        {
          info: {
            id: "ng_Ena_006",
            cardName: "[허밋 서처]",
            attribute: "cute",
            gachaType: "한정",
            gachaPoolName: "떠도는 은둔자의 여정 뽑기",
            eventName: "나의 발자취 너의 목적지",
            skillType: "판강",
            releaseDate: "2023-02-27",
          },
          media: {
            gachaBannerPath: "/gachas/23y/banner_23_09_n.png",
            eventBannerPath: "/events/23y/banner_23_09_n.png",
            songName: "로워",
            songJacketPath: "/jacket/23y/Lower_one-s_eyes.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "시킹 클록",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_007",
            cardName: "[괴로움도 비참함도]",
            attribute: "cool",
            gachaType: "통상",
            gachaPoolName: "번지는 색채에 빠져들어 뽑기",
            eventName: "공백의 캔버스에 그리는 나는",
            skillType: "스업",
            releaseDate: "2023-06-05",
          },
          media: {
            gachaBannerPath: "/gachas/23y/banner_23_23_n.png",
            eventBannerPath: "/events/23y/banner_23_23_n.png",
            songName: "노매드",
            songJacketPath: "/jacket/23y/Nomad.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "팡튀르 클로스",
          },
        },
        {
          info: {
            id: "ng_Ena_008",
            cardName: "[집게 힘이 너무 약해!]",
            attribute: "happy",
            gachaType: "한정",
            gachaPoolName: "Sporty Girl♡ 뽑기",
            eventName: "다 함께 엔조이! 스포조이 파크",
            skillType: "힐",
            releaseDate: "2023-08-19",
          },
          media: {
            gachaBannerPath: "/gachas/23y/banner_23_33.png",
            eventBannerPath: "/events/23y/banner_23_33.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "러블리 프릴 유니폼",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_009",
            cardName: "[추억의 동물은]",
            attribute: "mysterious",
            gachaType: "통상",
            gachaPoolName: "Colorful Paint 뽑기",
            eventName: "좋아하는 걸 그려 봐♪ 레인보우 캔버스",
            skillType: "스업",
            releaseDate: "2023-10-02",
          },
          media: {
            gachaBannerPath: "/gachas/23y/banner_23_40.png",
            eventBannerPath: "/events/23y/banner_23_40.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "그림 선생님☆ 스타일",
          },
        },
        {
          info: {
            id: "ng_Ena_010",
            cardName: "[그리고 싶어지는 표정]",
            attribute: "mysterious",
            gachaType: "통상",
            gachaPoolName: "portray yourself 뽑기",
            eventName: "소망은, 언젠가 아침을 넘어",
            skillType: "힐",
            releaseDate: "2023-11-25",
          },
          media: {
            gachaBannerPath: "/gachas/23y/banner_23_47_n.png",
            eventBannerPath: "/events/23y/banner_23_47_n.png",
            songName: "I랍니다",
            songJacketPath: "/jacket/23y/I_nan_desu.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "다크 데커레이트 스위트 걸",
          },
        },
        {
          info: {
            id: "ng_Ena_011",
            cardName: "[리그렛 르팡티르]",
            attribute: "cute",
            gachaType: "페스",
            gachaPoolName: "[2024 새해맞이] 컬러풀 페스티벌 뽑기",
            skillType: "굿스업",
            releaseDate: "2023-12-31",
          },
          media: {
            gachaBannerPath: "/gachas/24y/banner_24_f08.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "나르키소스 살로페트",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_012",
            cardName: "[부디 평온한 시간을]",
            attribute: "pure",
            gachaType: "통상",
            gachaPoolName: "독니를 내비치는 어두운 그림자 뽑기",
            eventName: "이미서블 디스코드",
            skillType: "판강",
            releaseDate: "2024-03-21",
          },
          media: {
            gachaBannerPath: "/gachas/24y/banner_24_09_n.png",
            eventBannerPath: "/events/24y/banner_24_09_n.png",
            songName: "잠자",
            songJacketPath: "/jacket/24y/Samsa.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "프러보크 크리피",
          },
        },
        {
          info: {
            id: "ng_Ena_013",
            cardName: "[느낀 것을 그림으로]",
            attribute: "happy",
            gachaType: "한정",
            gachaPoolName: "하늘하늘 흔들리는 소녀 뽑기",
            eventName: "교향하는 거리의 한편에서",
            skillType: "판강",
            releaseDate: "2024-04-04",
          },
          media: {
            gachaBannerPath: "/gachas/24y/banner_24_10.png",
            eventBannerPath: "/events/24y/banner_24_10.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "새벽녘 소녀의 날개옷",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_014",
            cardName: "[보이지 않는 그림자]",
            attribute: "mysterious",
            gachaType: "통상",
            gachaPoolName: "Where is ♡? 뽑기",
            eventName: "가면 쓴 나에게 작별을",
            skillType: "판강",
            releaseDate: "2024-07-11",
          },
          media: {
            gachaBannerPath: "/gachas/24y/banner_24_20_n.png",
            eventBannerPath: "/events/24y/banner_24_20_n.png",
            songName: "연극",
            songJacketPath: "/jacket/24y/Engeki.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Become an ally",
          },
        },
        {
          info: {
            id: "ng_Ena_015",
            cardName: "[즐거운 오후와 보이지 않는 미래]",
            attribute: "cute",
            gachaType: "월링",
            gachaPoolName: "Heart in Water 뽑기",
            eventName: "수면 아래 그림자를 찾아",
            skillType: "스업",
            releaseDate: "2024-11-08",
          },
          media: {
            gachaBannerPath: "/gachas/24y/banner_24_32_n.png",
            eventBannerPath: "/events/24y/banner_24_32_n.png",
            songName: "트와일라잇 라이트",
            songJacketPath: "/jacket/24y/Twilight_Light.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Twilight Draw",
          },
        },
        {
          info: {
            id: "ng_Ena_016",
            cardName: "[feat. 쿠로미]",
            attribute: "cool",
            gachaType: "콜라보",
            gachaPoolName: "SEKAI에서 Hello♡ 멋진 만남 뽑기",
            skillType: "힐",
            releaseDate: "2024-12-05",
          },
          media: {
            gachaBannerPath: "/gachas/collab/banner_c2.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "쿨한 소녀 원피스",
          },
        },
        {
          info: {
            id: "ng_Ena_017",
            cardName: "[호화로운 메이크업]",
            attribute: "mysterious",
            gachaType: "한정",
            gachaPoolName: "포토제닉♡ 밸런타인 뽑기",
            eventName: "Take the Best Shot!",
            skillType: "힐",
            releaseDate: "2025-01-31",
          },
          media: {
            gachaBannerPath: "/gachas/25y/banner_25_04.png",
            eventBannerPath: "/events/25y/banner_25_04.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "시크한 어른 비터 초콜릿",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_018",
            cardName: "[그림에서 보인 마음]",
            attribute: "pure",
            gachaType: "통상",
            gachaPoolName: "Drawing Blooming 뽑기",
            eventName: "Knowing the Unseen",
            skillType: "스업",
            releaseDate: "2025-04-10",
          },
          media: {
            gachaBannerPath: "/gachas/25y/banner_25_11_n.png",
            eventBannerPath: "/events/25y/banner_25_11_n.png",
            songName: "나는 비",
            songJacketPath: "/jacket/25y/Im_the_Rain.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "리졸브 원피스",
          },
        },
        {
          info: {
            id: "ng_Ena_019",
            cardName: "[가슴이 시릴듯한 노을 아래]",
            attribute: "cute",
            gachaType: "통상",
            gachaPoolName: "썩어가는 꽃은 이윽고 뽑기",
            eventName: "가시밭길은 어디로",
            skillType: "스업",
            releaseDate: "2025-10-12",
          },
          media: {
            gachaBannerPath: "/gachas/25y/banner_25_29_n.png",
            eventBannerPath: "/events/25y/banner_25_29_n.png",
            songName: "둔갑의 꽃",
            songJacketPath: "/jacket/25y/BAKENOHANA.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "우연히 떨어진 꽃",
          },
        },
        {
          info: {
            id: "ng_Ena_020",
            cardName: "[상처투성이가 되었더라도]",
            attribute: "cool",
            gachaType: "한정",
            gachaPoolName: "That flower, once more 뽑기",
            eventName: "상처투성이 손으로, 우리는",
            skillType: "퍼스업",
            releaseDate: "2025-11-30",
          },
          media: {
            gachaBannerPath: "/gachas/25y/banner_25_34_n.png",
            eventBannerPath: "/events/25y/banner_25_34_n.png",
            songName: "여화에 넋을 잃고",
            songJacketPath: "/jacket/25y/Yoka_ni_Mitorete.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "시즈 베스티",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_021",
            cardName: "[가방 가득한 마음]",
            attribute: "cool",
            gachaType: "통상",
            gachaPoolName: "Obscurity Parade 뽑기",
            eventName: "Unreliable Notes",
            skillType: "힐",
            releaseDate: "2026-03-20",
          },
          media: {
            gachaBannerPath: "/gachas/26y/banner_26_09_n.png",
            eventBannerPath: "/events/26y/banner_26_09_n.png",
            songName: "D/N/A",
            songJacketPath: "/jacket/26y/DNA.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Papilionidae",
          },
        },
        {
          info: {
            id: "ng_Ena_022",
            cardName: "[항상, 네가]",
            attribute: "mysterious",
            gachaType: "월링",
            gachaPoolName: "Night's Treasure 뽑기",
            eventName: "물거품에 감싸여",
            skillType: "스업",
            releaseDate: "2026-06-09",
          },
          media: {
            gachaBannerPath: "/gachas/26y/banner_26_17_n.png",
            eventBannerPath: "/events/26y/banner_26_17_n.png",
            songName: "노매드",
            songJacketPath: "/jacket/23y/Nomad.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Brush of Solitude",
          },
        },
        {
          info: {
            id: "ng_Ena_023",
            cardName: "[그 그림을 함께]",
            attribute: "pure",
            gachaType: "페스",
            gachaPoolName: "블룸 페스티벌 뽑기",
            eventName: "Deeper and deeper",
            skillType: "블페",
            releaseDate: "2026-07-07",
          },
          media: {
            gachaBannerPath: "/gachas/26y/banner_26_f18.png",
            eventBannerPath: "/events/26y/banner_26_19_w.png",
            songName: "아이리드",
            songJacketPath: "/jacket/26y/EYELID.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Casual Painter",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_024",
            cardName: "[열렬한 환영에 우물쭈물!]",
            attribute: "happy",
            gachaType: "통상",
            gachaPoolName: "너를 향해 웃는 fuzzy daisy 뽑기",
            eventName: "풀리지 않는 내일에 손을 뻗어서",
            skillType: "퍼스업",
            releaseDate: "2026-08-22",
          },
          media: {
            gachaBannerPath: "/gachas/26y/banner_26_24_n.png",
            eventBannerPath: "/events/26y/banner_26_24_n.png",
            songName: "장식해",
            songJacketPath: "/jacket/26y/Kazatte.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Petit joie",
          },
        },
        {
          info: {
            id: "ng_Ena_025",
            cardName: "[자신의 경계를 넘어]",
            attribute: "pure",
            gachaType: "한정",
            gachaPoolName: "Hues of the Self 뽑기",
            eventName: "Colors of Pure Sense",
            skillType: "판강",
            releaseDate: "2026-10-31",
          },
          media: {
            gachaBannerPath: "/gachas/26y/jbanner_26_30_n.png",
            eventBannerPath: "/events/26y/jbanner_26_30_n.png",
            songName: "그 그림의 이름은",
            songJacketPath: "/jacket/26y/Sono_E_no_Namae_wa.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "아스필레 에이프런",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_026",
            cardName: "[해 질 녘의 창가]",
            attribute: "happy",
            gachaType: "통상",
            gachaPoolName: "¿행복한 발레리나? 뽑기",
            eventName: "깨어나지 않는 환상을 노크하며",
            skillType: "힐",
            releaseDate: "2027-01-11",
          },
          media: {
            gachaBannerPath: "/gachas/27y/jbanner_27_02_n.png",
            eventBannerPath: "/events/27y/jbanner_27_02_n.png",
            songName: "행복형",
            songJacketPath: "/jacket/27y/Koufukukei.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "위시 튀튀",
          },
        },
        {
          info: {
            id: "ng_Ena_027",
            cardName: "[악몽의 세계에 울려퍼지는 노래는]",
            attribute: "happy",
            gachaType: "한정",
            gachaPoolName: "Lunatic doll in the box 뽑기",
            eventName: "악몽의 정원을 노래로 밝히며",
            skillType: "스업",
            releaseDate: "2027-02-28",
          },
          media: {
            gachaBannerPath: "/gachas/27y/jbanner_27_07.png",
            eventBannerPath: "/events/27y/jbanner_27_07.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Lunatic Pupperteer",
            hasHair: true,
          },
        },
        {
          info: {
            id: "ng_Ena_028",
            cardName: "[쪽빛으로 피는 꽃]",
            attribute: "mysterious",
            gachaType: "통상",
            gachaPoolName: "투명하게 밝아오는 기억의 한가운데서 뽑기",
            eventName: "바래지 않는 지금을, 물들여",
            skillType: "퍼스업",
            releaseDate: "2027-07-09",
          },
          media: {
            gachaBannerPath: "/gachas/27y/jbanner_27_20_n.png",
            eventBannerPath: "/events/27y/jbanner_27_20_n.png",
            songName: "빛",
            songJacketPath: "/jacket/27y/Hikari.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "cherished moment",
          },
        },
        {
          info: {
            id: "ng_Ena_029",
            cardName: "[그때 말하지 못했던 것]",
            attribute: "cool",
            gachaType: "월링",
            gachaPoolName: "Crawl out of Vibrant hell 뽑기",
            eventName: "Again And Again Ambition!",
            skillType: "스업",
            releaseDate: "2027-08-17",
          },
          media: {
            gachaBannerPath: "/gachas/27y/jbanner_27_24.png",
            eventBannerPath: "/events/27y/jbanner_27_24.png",
            songName: "안티 재능",
            songJacketPath: "/jacket/27y/ANTI_SAINOU.png",
          },
          costume: {
            hasCostume: true,
            costumeName: "Painting ambition",
            hasHair: true,
          },
        },
  ]
});

// 🌟 에나 카드를 배열로 내보내기! (나중에 전체 카드 DB에 합칠 때 사용)
export const EnaCards = [
  ...EnaNiigo,
];